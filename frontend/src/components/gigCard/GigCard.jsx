...
const GigCard = (props) => {
  const [isHovered, setIsHovered] = useState(false);
  const gig = props.item || {};
  const freelancer = gig.freelancer_id || {};
  const verification = freelancer.full_verification?.[0] || {};
  const sellerName = verification.full_name || freelancer.username || "Liverr Seller";
  const sellerAvatar =
    verification.profile_pic ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(sellerName)}&background=fee2e2&color=991b1b`;
  ...
  const calculateAverageRating = () => {
    ...
    return (totalRating / gig.reviews.length).toFixed(1);
  };
  ...
  // Convert USD to PKR (using an approximate exchange rate of 1 USD = 280 PKR)
  const priceInPKR = Math.round((gig.price || 0) * 280);
  ...
  }, [gigId]); // Re-calculate only if the gig item changes
  
  return (
    <Link to={`/gig/${gigId}`} className="no-underline">
      <div 
      ...
      >
        ...
        <div className="relative">
          <img
            src={gig.coverimage || gig.images?.[0] || "https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg?auto=compress&cs=tinysrgb&w=1200"}
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
            <div className="flex items-center">
              ...
              <span>{gig.delivery_time || 1} day delivery</span>
            </div>
            <div className="flex items-center">
              <FontAwesomeIcon icon={faCheck} className="text-red-500 mr-1" />
            ...
            <div className="flex justify-end items-center">
              <div className="text-right">
                ...
                <h5 className={`text-lg font-bold transition-colors duration-300 ${isHovered ? 'text-red-500' : 'text-black'}`}>
                  {isHovered ? `PKR ${priceInPKR}` : `$${gig.price || 0}`}
                </h5>
              </div>
            </div>
          </div>
